...
```text
affiliation_b = affiliation_0 + a_1 warmth + a_2 optimism - a_3 pessimism
dominance_b   = dominance_0 + a_4 assertiveness - a_5 shyness
reactivity_p  = reactivity_0 * (1 + a_6 volatility + a_7 shyness - a_8 calmness)
```
...
```text
Q_t = 2^(-Δt / H_q) * Q_(t-1) + impulse(E_t, X_t, appraisal_t)
```
...
```text
anger_push = combo(-V, A, D, max(-G, C))
cold_war = combo(-V, -A, -S, max(-K, -G))
anxious_withdraw = combo(-V, A, -D, -K)
repair = combo(-V, S, max(K, 0.25), 1 - uncertainty_penalty)
```
...
```text
R_t = relationship_decision_llm(I_t)
R_t.decision in {forgive, repair, boundary, cold_war, escalate, none}
```
...
```text
C_t = conflict_analysis_llm(I_t)
C_t.cause in {user_fault, bot_whim, bot_misread, mutual, external, none}
```
...
```text
repair_signal_t = max(
  apology_sincerity_t * 1[user_acknowledged_t],
  repair_quality_t * 1[repaired_t]
)

grievance_t = clip(
  fault_severity_t * (1 - repair_signal_t)
  + 0.35 * repeat_offense_t,
  0, 1
)

self_correction_t = max(
  bot_whim_level_t * 1[cause in {bot_whim, bot_misread}],
  repair_signal_t * 1[cause in {user_fault, mutual}]
)
```
...
```text
grievance_t = clip(
  0.55 fault_severity_t
  + 0.18 intent_t
  + 0.16 avoid_t
  + 0.16 trust_t
  + 0.12 face_threat_t
  + 0.10 expectation_violation_t
  + 0.16 boundary_t
  + 0.20 repeat_offense_t
  + 0.14 residue_t
  - 0.40 repair_signal_t
  - 0.24 forgive_t
  - 0.30 misread_t
  - 0.18 amb_t,
  0, 1
