...
The self matrices use diagonal values `(0.30, 0.35, 0.40)` for fast/mid/slow. The only off-diagonal couplings are safety and affiliation into valence `+0.06`, uncertainty into arousal `+0.06`, safety into uncertainty `-0.06`, affiliation into agency `+0.05`, and agency into expression pressure `+0.05`; mid uses 5/6 and slow 1/2 of those couplings. `U_fast=0.80I`, `U_mid=0.75I`, `U_slow=0.65I`. The formula validator rejects the manifest unless interval/Jacobian analysis proves `sup ||J||2 < 0.995`.
...
@pytest.mark.parametrize("case,expected", [
    ("ordinary", "SPEAK"), ("silent", "HOLD"), ("fallback", "UNKNOWN"), ("tool_loop", "UNKNOWN"),
...
- [ ] SILENT is terminal HOLD evidence. Only structured `ReplyKind.SPEAK` becomes a candidate; FALLBACK, tool/attachment/partial/cancelled/missing provenance remains UNKNOWN. Ordinary framework delivery uses `after_message_sent`; segmented delivery requires its own all-segments-success callback, and second-segment failure stays UNKNOWN. CLARIFY requires a future explicit marker. Proactive REACH requires structured `dispatched=True`.
...
- [ ] **RED:** Assert length-framed HMAC distinguishes `("qq","a:b")` from `("qq:a","b")`; raw IDs never appear in filenames/state/trace. Assert duplicate response claims, object-identity lookup, FIFO guessing, and stale TTL entries are rejected.
