...
人格向量 $\boldsymbol{\pi} \in [0,1]^5$ 基于大五人格模型（OCEAN）：开放性（O）、尽责性（C）、外向性（E）、宜人性（A）、神经质（N）。

人格向量来源：
\begin{enumerate}[label=(\alph*)]
\item \textbf{初始化}：由角色设定映射到 $[0,1]^5$。
\item \textbf{漂移}：长期交互中由伤痕历史和空洞模式驱动缓慢漂移。
\item \textbf{约束}：漂移速率受自创生边界（L6）保护，上界 $\eta \leq 10^{-4}$。
\end{enumerate}

\subsection{人格派生参数化（Personality-Derived Parameterization）}
\label{subsec:personality-parameterization}

所有计算层参数由 $\boldsymbol{\pi}$ 统一派生：
\begin{equation}
...
\begin{itemize}
\item $E$ $\to$ 表达阈值、伤痕阈值
\item $N$ $\to$ 愈合速率、门控灵敏度、空洞检测阈值
\item $A$ $\to$ 呈现矩阵一致性界 $\kappa$
\item $O$ $\to$ HGT 注意力先验
\item $C$ $\to$ 记忆深度、边界修复速率
\end{itemize}

\subsection{人格反馈闭环（Personality Feedback Loop）}
...
\end{equation}
其中 $\eta = \eta_0 \cdot (1 - \text{boundary\_stability})$。

具体驱动：
\begin{enumerate}
\item 伤痕密度 $\to$ $N\!\uparrow$
\item 空洞压力 $\to$ $O\!\downarrow$
\item coherence $\to$ $A$（正相关）
\item 表达反馈 $\to$ $E$（accepted$\uparrow$, ignored$\downarrow$）
\item $H^1$ 不一致性 $\to$ $C\!\downarrow$
\end{enumerate}

关键性质：有阻尼的非线性反馈环。边界稳定时漂移近零，相变时漂移率暂时升高。
...
\caption{表达力分离：可区分输出状态数 vs.\ 伤疤数 $k$。伤疤代数（红，$4^k$ 增长）对单一固定输入产生指数级多的不同输出；固定算子基线（蓝）始终只产生 1 种。}
...
\caption{空洞检测率 vs.\ 惊奇度水平。从 $\sim$0\% 到 100\% 的平滑 sigmoid 过渡，每个水平 50 次试验，高斯噪声 $\sigma = 0.15$。阴影区域显示检测阈值附近的过渡带。}
\label{fig:detection}
\end{figure}
...
\caption{三态时间签名（3 面板：压力、深度、边界）30 tick。从未（平坦零值）、已解决（低稳定值）和回避中（增长的压力/深度与非零边界）在所有时间点两两可区分。}
...
\caption{滞后效应演示。两条路径（先伤害 vs.\ 先安慰）在分歧后接收相同共享事件，却保持 $\sim$20 个伤疤的永久分离。路径依赖不可逆。}
\label{fig:hysteresis}
\end{figure}

\subsection{实验 5：消融研究}

使用 ComputationSpine 处理 100 个多样化输入事件，测量状态轨迹丰富度（访问的唯一状态向量数），在五种条件下进行：完整系统、无空洞（空洞演算禁用）、无伤疤（伤疤代数禁用）、无耦合（$\Gamma$ 和 $\Phi$ 禁用）、无 HGT（异构图 Transformer 旁路）。
...
\item 移除 HGT 降低决策融合质量但保留核心动态。
\end{itemize}

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\columnwidth]{../experiments/figures/fig5_ablation.png}
\caption{消融：100 个多样化输入下 5 种条件（完整、无空洞、无伤疤、无耦合、无 HGT）的状态轨迹丰富度。所有组件都对完整系统的表达力有贡献。}
\label{fig:ablation}
\end{figure}
...
\caption{1000 tick 长期稳定性。上：平均伤疤调制因子收敛至 0。中：伤疤计数线性增长（$\sim$3.14/tick）。下：空洞计数收敛于 $\sim$29。系统达到稳定均衡而无发散。}
\label{fig:stability}
\end{figure}
...
构造具有高神经质人格（$\kappa$ 大）和 $N=3$ 个关系的智能体。呈现矩阵 $P_i$ 被冻结以诱导矛盾的自我呈现需求。智能体在 200 tick 内接收跨关系的矛盾事件。追踪不一致性能量（$\|\delta^0 \mathbf{x}\|^2$）、解离压力和 $\dim H^1(K, \mathcal{F})$。\emph{结果：}不一致性能量呈 S 曲线从 0 增长至 $\approx 65$；解离压力从 0.19 增长至 0.59。然而 $\dim H^1$ 始终保持为 0，因为呈现矩阵保持满秩（系统通过内部状态调整而非产生真正的上同调阻碍来解决矛盾）。这表明高不一致性能量和解离压力是 $H^1 > 0$ 的必要但非充分条件（图~\ref{fig:cohomological_dissociation}）。
...
\paragraph{愈合速率假设。}愈合持续时间（$T(raw)=10$，$T(closing)=40$，$T(scarred)=150$）由人格向量实时派生（神经质 $\to$ 愈合速率，外向性 $\to$ 伤疤阈值，宜人性 $\to$ 一致性界），通过仿射映射加截断实现。虽然这消除了对训练数据的需求，但派生函数本身是设计的而非经纵向人类数据实证验证的。
...
\begin{itemize}
\item \textbf{人格派生参数：}所有愈合速率和阈值当前由人格向量实时派生（神经质 $\to$ 愈合速率，外向性 $\to$ 伤疤阈值，宜人性 $\to$ 一致性界），通过 \texttt{derive\_params()} 实现，无需训练数据。未来工作可探索从纵向交互数据微调这些派生函数是否能提高生态效度。
