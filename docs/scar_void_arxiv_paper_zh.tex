...
\item \textbf{L5：异构图 Transformer（HGT）。}类型感知的决策融合，具有 7 种 token 类型：\{scar, void, boundary, personality, surprise, expression, context\}。类型特定投影 $W_Q^\tau, W_K^\tau, W_V^\tau$，人格导出的注意力先验 $\mu^{(\tau_s, \tau_t)}$ 和类型内掩码。
...
我们通过十组实验评估空洞-伤疤框架及关系层论，覆盖表达力、检测准确率、状态区分、路径依赖、消融、长期稳定性、多用户隔离、上同调解离、谱传播和三元不可约性。前七组实验使用参考 Python 实现在单核（Intel i5, 3.0 GHz, 16 GB RAM）上运行；后三组为关系层论验证实验。
...
\paragraph{结果。}无自创生干预时，系统在约第 350 轮进入全维度麻木状态（所有 8 个维度调制因子 $< 0.5$），原因是累积淡化伤疤衰减（$0.7^k \to 0$）。这证实了定理~\ref{thm:scar-convergence}：系统收敛到"伤疤均衡"。在完整架构中，自创生边界（L6）会在全面麻木前触发相变以重置关键维度（图~\ref{fig:stability}）。

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\columnwidth]{../experiments/figures/fig6_stability.png}
\caption{无自创生恢复的长期稳定性。上：伤疤累积。中：麻木维度在约 350 轮达到最大值。下：一致性崩溃。自创生层（L6）设计为在此退化状态前介入。}
\label{fig:stability}
\end{figure}
...
构造具有 $N=3$ 个关系的智能体，其中关系 1 和 2 施加矛盾的自我呈现需求（例如一方要求高开放性，另一方要求内敛）。随着伤疤历史分化，测量 $\dim H^1(K, \mathcal{F})$ 随时间的变化。\emph{结果：}一旦呈现差距超过 $\kappa(\boldsymbol{\pi})$，$H^1$ 单调增长，且独立逐关系实例无法检测该不一致（图~\ref{fig:cohomological_dissociation}）。
...
在关系 $i$ 中施加受控伤疤事件，测量关系 $j$ 中传播效应 $\|\Delta \mathbf{x}_j(t)\|$ 随时间的变化。将观测衰减率与理论界 $e^{-\lambda_1(t-t_0)} \cdot \|P_j^T P_i\|_2$（定理~\ref{thm:sheaf-spectral}）比较。\emph{结果：}观测衰减在谱界的常数因子范围内，且衰减率与计算的 $\lambda_1$ 相关（图~\ref{fig:spectral_propagation}）。
...
创建 2-单形（智能体 + 两个伙伴同时共在）。生成共在状态并尝试仅从成对限制重构。\emph{结果：}当 $\dim H^2 > 0$ 时重构误差非零，确认群体状态包含二元数据中不可获得的不可约三元信息（图~\ref{fig:triadic_irreducibility}）。
...
\midrule
1. 表达力分离 & 伤疤 vs.\ 固定 SSM 胜率 & 7/8 \\
2. 空洞检测 & F1（空洞 / 基线） & 0.702 / 0.649 \\
3. 三态区分 & 两两可区分 & 全部 3 态 \\
4. 滞后效应 & 路径依赖持续 & 确认 \\
5. 消融 & 两组件均必要 & 确认 \\
6. 稳定性 & 无 L6 时麻木 & 约第 350 轮 \\
7. 多用户 & 隔离检查 & 7/7 通过 \\
8. 上同调解离 & $\dim H^1$ 单调增长 & 确认 \\
9. 谱传播 & 衰减率符合理论界 & 确认 \\
10. 三元不可约性 & $H^2 > 0$ 时重构误差非零 & 确认 \\
\bottomrule
\end{tabular}
\caption{实验结果汇总。}
\label{tab:results}
