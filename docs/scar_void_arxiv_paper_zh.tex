...
\item \textbf{边界通透（$\pi_{\text{perm}}$）：}HGT 注意力先验分布（高 $\pi_{\text{perm}}$ $\to$ 更均匀的先验，对多样信号类型更具接受性）。
...
\caption{人格反馈闭环验证。三种条件下人格向量的漂移轨迹：(A) 反复受伤（$\pi_{\text{acuity}}\!\uparrow$，$\pi_{\text{perm}}\!\downarrow$），(B) 持续接纳（$\pi_{\text{expr}}\!\uparrow$，$\pi_{\text{grav}}\!\uparrow$），(C) 跨关系矛盾（$\pi_{\text{order}}\!\downarrow$）。漂移有界且由计算状态因果驱动。}
