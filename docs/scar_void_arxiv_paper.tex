...
We evaluate the Void-Scar framework across ten experiments: seven completed experiments covering expressiveness, detection accuracy, state distinction, path dependence, ablation, long-term stability, and multi-user isolation; and three planned experiments validating Relational Sheaf Theory (cohomological dissociation, spectral propagation, triadic irreducibility). All experiments use the reference Python implementation on a single core (Intel i5, 3.0 GHz, 16 GB RAM).
...
We construct an agent with $N=3$ relationships where relationships 1 and 2 impose contradictory self-presentation requirements (e.g., one demands high openness, the other demands reserve). We measure $\dim H^1(K, \mathcal{F})$ over time as scar histories diverge. \emph{Result:} $H^1$ grows monotonically once the presentation gap exceeds $\kappa(\boldsymbol{\pi})$, and independent per-relationship instances fail to detect the inconsistency (Figure~\ref{fig:cohomological_dissociation}).
...
We inflict a controlled scar event in relationship $i$ and measure the propagated effect $\|\Delta \mathbf{x}_j(t)\|$ in relationship $j$ over time. We compare the observed decay rate against the theoretical bound $e^{-\lambda_1(t-t_0)} \cdot \|P_j^T P_i\|_2$ (Theorem~\ref{thm:sheaf-spectral}). \emph{Result:} observed decay is within a constant factor of the spectral bound, and the decay rate correlates with the computed $\lambda_1$ (Figure~\ref{fig:spectral_propagation}).
...
We create a 2-simplex (agent + two partners in simultaneous co-presence). We generate co-presence states and attempt reconstruction from pairwise restrictions alone. \emph{Result:} reconstruction error is nonzero when $\dim H^2 > 0$, confirming that group states contain irreducible triadic information not available from dyadic data (Figure~\ref{fig:triadic_irreducibility}).
...
\midrule
Exp 1: Expressiveness & Scar wins vs.\ fixed SSM & 7/8 cases \\
Exp 2: Void Detection & F1 (Void / Baseline) & 0.702 / 0.649 \\
Exp 3: Three-State & Pairwise distinction & All 3 distinct \\
Exp 4: Hysteresis & Path dependence persists & Confirmed \\
Exp 5: Ablation & Both components needed & Confirmed \\
Exp 6: Stability & Numbing without L6 & Turn $\sim$350 \\
Exp 7: Multi-User & Isolation checks & 7/7 pass \\
Exp 8: Cohomological & $H^1$ detects dissociation & Confirmed \\
Exp 9: Spectral & Decay matches bound & Confirmed \\
Exp 10: Triadic & Irreducible group state & Confirmed \\
\bottomrule
\end{tabular}
