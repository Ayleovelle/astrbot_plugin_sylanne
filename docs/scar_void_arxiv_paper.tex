...
\item Medium surprise ($0.15$--$0.45$): \emph{normal path}---full Void-Scar + MoE-HGT.
...
\item \textbf{Boundary Permeability ($\MoE-HGT attention prior distribution (high $\pi_{\text{perm}}$ $\to$ more uniform priors, receptive to diverse signal types).
...
\item $\MoE-HGT attention prior: higher boundary permeability produces more uniform attention distributions, increasing receptivity to diverse signal types.
...
\caption{Expressiveness separation: L2 state divergence between wounded system and baseline increases monotonically with number of injuries. After 7 injuries, mean divergence reaches 0.049, proving scars produce irreversible state separation.}
...
\caption{Void detection driven by personality. High perception acuity (0.8) produces more voids (9--10), low acuity (0.2) produces fewer (5--6). Detection sensitivity is personality-driven rather than threshold-driven.}
\label{fig:detection}
\end{figure}
...
\caption{Three-state separation via void depth: never-discussed (depth=0.20), resolved (depth=0.30), actively-avoided (depth=6.90, a 34$\times$ difference). States are cleanly separable by a single scalar.}
...
\caption{Hysteresis demonstration. Two systems with different injury histories continue to diverge after receiving identical subsequent inputs (final divergence = 0.104). 47 vs.\ 51 scars form, proving path dependence is ineliminable.}
\label{fig:hysteresis}
\end{figure}

\subsection{Exp 5: Ablation Study}

We instantiate a ComputationSpine with 100 diverse input events and measure state trajectory richness (number of unique state vectors visited) under five conditions: Full system, No Void (void calculus disabled), No Scar (scar algebra disabled), No Coupling ($\Gamma$ and $\Phi$ disabled), and No MoE-HGT (heterogeneous graph transformer bypassed).
...
\item Removing MoE-HGT reduces decision fusion quality but preserves core dynamics.
\end{itemize}

\begin{figure}[H]
\centering
\includegraphics[width=0.7\columnwidth]{../experiments/figures/fig5_ablation.png}
\caption{Ablation: MoE-HGT removal has the largest impact ($-23\%$), as the decision-generation layer. Scar/Void/Coupling are regulatory layers---removal loses constraints but not generative capacity. Each component has a unique contribution signature.}
\label{fig:ablation}
\end{figure}
...
\caption{Long-term stability over 1000 ticks under mixed stress. Ground-state norm stays bounded (0.25), 10 scars form, at most 11 simultaneously active voids, no NaN/Inf. Log-compression and safety mechanisms guarantee long-term stability.}
\label{fig:stability}
\end{figure}
...
\caption{Personality feedback loop verification (Dual-EMA). Sustained acceptance $\to$ expression drive $+0.226$; repeated wounding $\to$ expression drive $+0.091$, perception acuity $-0.101$; cross-relational contradiction $\to$ relational gravity $-0.050$. Drift is damped; single events do not alter the skeleton.}
