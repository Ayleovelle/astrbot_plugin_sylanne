...
The coupled Void-Scar system is embedded in a six-layer computation stack designed for sub-10ms end-to-end latency on commodity hardware.

\subsection{Six-Layer Stack}
...
\item \textbf{L4: Heterogeneous Graph Transformer (HGT).} Type-aware decision fusion with 7 token types: \{scar, void, boundary, personality, surprise, expression, context\}. Type-specific projections $W_Q^\tau, W_K^\tau, W_V^\tau$ with personality-derived attention prior $\mu^{(\tau_s, \tau_t)}$ and intra-type masking.

\item \textbf{L5: Autopoietic Boundary.} 32-dimensional identity core vector. Incoming perturbations are assessed via orthogonal projection:
\begin{itemize}
\item Penetration $< 0.3$: absorb (elastic).
\item Penetration $0.3$--$0.7$: resist (boundary stressed, identity unchanged).
\item Penetration $\geq 0.7$: phase transition (identity rotates $\leq 6°$).
\end{itemize}

\item \textbf{L6: Phase Transition Expression.} Expression is discontinuous phase transition, not continuous function. Pressure accumulates; when exceeding personality-derived threshold, output jumps to one of three modes: \emph{hint} ($< 0.5$), \emph{normal} ($0.5$--$1.0$), \emph{urgent} ($> 1.0$).
\end{enumerate}
...
L4 HGT & skip & 0.40 ms & 0.40 ms \\
L5 Autopoiesis & 0.01 ms & 0.01 ms & 0.10 ms \\
L6 Phase Transition & $<$0.01 ms & $<$0.01 ms & $<$0.01 ms \\
\midrule
\textbf{Total} & \textbf{0.17 ms} & \textbf{3.0 ms} & \textbf{6.6 ms} \\
\bottomrule
\end{tabular}
\end{table}

Approximately 90\% of messages take the fast path (0.17ms); 10\% take the full path (6.6ms). All within the 10ms budget.
...
We evaluate the Void-Scar framework across six experiments covering expressiveness, detection accuracy, state distinction, path dependence, ablation, and long-term stability. All experiments use the reference Python implementation on a single core (Intel i5, 3.0 GHz, 16 GB RAM).

\subsection{Exp 1: Expressiveness Separation}

We construct adversarial timing sequences with $k$ wounding events spaced at either $T_1=9$ ticks (scar stays \emph{raw}) or $T_2=11$ ticks (scar advances to \emph{closing}), creating $2^k$ distinct timing patterns. After the wounding phase, a constant probe is applied and the output recorded.

\paragraph{Result.} Scar Algebra distinguishes up to 4 output states for $k \leq 8$, while a fixed-operator linear SSM with equivalent state budget ($1 + k$ dimensions) distinguishes at most 2. The self-modifying operator semantics (healing stages change $\alpha$) create path-dependent distinctions that fixed matrices cannot replicate. Scar Algebra wins in 7/8 cases (Figure~\ref{fig:expressiveness}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{../experiments/figures/fig1_expressiveness.png}
\caption{Expressiveness separation: distinguishable output states vs.\ number of scars $k$. Scar Algebra (red) consistently outperforms the fixed-operator baseline (blue).}
\label{fig:expressiveness}
\end{figure}

\subsection{Exp 2: Void Detection Precision/Recall}

We generate 200 synthetic conversations (32-byte HDC vectors per event). In 100, we plant avoidance patterns (approach a topic then suddenly deflect); the other 100 have normal topic transitions.

\paragraph{Result.} Void Calculus achieves F1 = 0.702 (precision 0.54, recall 1.00) vs.\ a fixed-threshold baseline F1 = 0.649 (precision 1.00, recall 0.48). Void Calculus catches all planted avoidance patterns at the cost of some false positives on noisy negatives; the baseline misses 52\% of moderate avoidance signals (Figure~\ref{fig:detection}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.65\columnwidth]{../experiments/figures/fig2_void_detection.png}
\caption{Void detection performance. Void Calculus trades precision for perfect recall; the baseline has perfect precision but misses half of avoidance patterns.}
\label{fig:detection}
\end{figure}

\subsection{Exp 3: Three-Way State Distinction}

We construct three scenarios: S1 (topic never discussed, void with $\delta=0$), S2 (topic resolved, ghost remains), S3 (topic actively avoided, void with $\delta > 0$ and pressure accumulating). We measure behavioral signatures across 7 metrics.

\paragraph{Result.} All three states are pairwise distinct in Void Calculus. A Bayesian baseline collapses S1 and S3 to the same posterior $P = 0.10$, confirming that Void Calculus provides strictly richer state representation (Figure~\ref{fig:three_states}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{../experiments/figures/fig3_three_states.png}
\caption{Three-state behavioral signatures. Each state has a unique profile; Bayesian baselines cannot distinguish S1 from S3.}
\label{fig:three_states}
\end{figure}

\subsection{Exp 4: Hysteresis (Path Dependence)}

Two VoidScarEngine instances receive different histories then the same probe: Path A (direct discussion) vs.\ Path B (avoidance $\to$ void pressure $\to$ coupling $\Gamma$ fires $\to$ resolution $\to$ probe).

\paragraph{Result.} After resolution, Path B retains: 1 scar (modifier = 1.5), 1 ghost (heightened sensitivity), and different probe response (L1 distance 0.15 vs.\ 0.10). Both paths end with zero active voids, yet the system permanently remembers the avoidance history (Figure~\ref{fig:hysteresis}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\columnwidth]{../experiments/figures/fig4_hysteresis.png}
\caption{Hysteresis demonstration. Left: two paths to the same topic. Right: response differs permanently due to scar residue and ghost sensitivity.}
\label{fig:hysteresis}
\end{figure}

\subsection{Exp 5: Ablation Study}

We run a standardized 200-event sequence through five configurations: full system, $-$Scar, $-$Void, $-$Coupling, and $-$Both (plain $\tanh$ evolution).

\paragraph{Result.} See Figure~\ref{fig:ablation}. Key findings:
\begin{itemize}
\item Removing scars eliminates coherence variation (coherence always 1.0).
\item Removing voids eliminates three-state distinction.
\item Removing both eliminates hysteresis entirely.
\item Coupling removal alone does not degrade capabilities---coherence still varies because the metric measures alignment, not the coupling mechanism itself.
\end{itemize}

\begin{figure}[ht]
\centering
\includegraphics[width=0.7\columnwidth]{../experiments/figures/fig5_ablation.png}
\caption{Ablation heatmap. Green = capability present, red = absent. Both Scar and Void are necessary; coupling is beneficial but not strictly required for the measured capabilities.}
\label{fig:ablation}
\end{figure}

\subsection{Exp 6: Long-Term Stability}

We simulate 1500 conversation turns with realistic input distribution (60\% normal, 20\% emotional, 10\% wounding, 5\% avoidance, 5\% resolution).

\paragraph{Result.} Without autopoietic intervention, the system enters a fully-numbed state (all 8 dimensions with modifier $< 0.5$) by turn $\sim$350 due to cumulative faded-scar attenuation ($0.7^k \to 0$). This confirms Theorem~\ref{thm:scar-convergence}: the system converges to a ``scarred equilibrium.'' In the full architecture, the autopoietic boundary (L5) would trigger a phase transition to reset critical dimensions before full numbing occurs (Figure~\ref{fig:stability}).

\begin{figure}[ht]
\centering
\includegraphics[width=0.85\columnwidth]{../experiments/figures/fig6_stability.png}
\caption{Long-term stability without autopoietic recovery. Top: scar accumulation. Middle: numbed dimensions reach maximum at $\sim$350 turns. Bottom: coherence collapses. The autopoiesis layer (L5) is designed to intervene before this degenerate state.}
\label{fig:stability}
\end{figure}
...
\subsection{Planned Experiments: Relational Sheaf Theory}

The following experiments are designed to validate the Relational Sheaf Theory (\S\ref{sec:sheaf}) and will be conducted with the multi-relational runtime.

\paragraph{Exp 8: Cohomological Dissociation Detection.}
Construct an agent with $N=3$ relationships where relationships 1 and 2 impose contradictory self-presentation requirements (e.g., one demands high openness, the other demands reserve). Measure $\dim H^1(K, \mathcal{F})$ over time as scar histories diverge. \emph{Hypothesis:} $H^1$ grows monotonically once the presentation gap exceeds $\kappa(\boldsymbol{\pi})$, and independent per-relationship instances fail to detect the inconsistency.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{../experiments/figures/fig7_cohomological_dissociation.png}
\caption{Cohomological dissociation detection. $\dim H^1$ grows monotonically as presentation gap exceeds threshold $\kappa(\boldsymbol{\pi})$; independent per-relationship instances fail to detect the inconsistency.}
\label{fig:cohomological_dissociation}
\end{figure}

\paragraph{Exp 9: Spectral Propagation Verification.}
Inflict a controlled scar event in relationship $i$ and measure the propagated effect $\|\Delta \mathbf{x}_j(t)\|$ in relationship $j$ over time. Compare the observed decay rate against the theoretical bound $e^{-\lambda_1(t-t_0)} \cdot \|P_j^T P_i\|_2$ (Theorem~\ref{thm:sheaf-spectral}). \emph{Hypothesis:} observed decay is within a constant factor of the spectral bound, and the decay rate correlates with the computed $\lambda_1$.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{../experiments/figures/fig8_spectral_propagation.png}
\caption{Spectral propagation verification. Observed decay rate (solid) closely tracks the theoretical spectral bound (dashed), confirming Theorem~\ref{thm:sheaf-spectral}.}
\label{fig:spectral_propagation}
\end{figure}

\paragraph{Exp 10: Triadic Irreducibility.}
Create a 2-simplex (agent + two partners in simultaneous co-presence). Generate co-presence states and attempt reconstruction from pairwise restrictions alone. \emph{Hypothesis:} reconstruction error is nonzero when $\dim H^2 > 0$, confirming that group states contain irreducible triadic information not available from dyadic data.

\begin{figure}[ht]
\centering
\includegraphics[width=0.75\columnwidth]{../experiments/figures/fig9_triadic_irreducibility.png}
\caption{Triadic irreducibility. Reconstruction error is nonzero when $\dim H^2 > 0$, confirming irreducible triadic information not available from dyadic data alone.}
...
\midrule
Expressiveness & Scar wins vs.\ fixed SSM & 7/8 cases \\
Void Detection & F1 (Void / Baseline) & 0.702 / 0.649 \\
Three-State & Pairwise distinction & All 3 distinct \\
Hysteresis & Path dependence persists & Confirmed \\
Ablation & Both components needed & Confirmed \\
Stability & Numbing without L5 & Turn $\sim$350 \\
Multi-User & Isolation checks & 7/7 pass \\
\bottomrule
\end{tabular}
...
\paragraph{Healing rate assumptions.} The fixed healing durations ($T(raw)=10$, $T(closing)=40$, $T(scarred)=150$) are design choices, not derived from data. This limitation was partially addressed in v1.0.1 with adaptive healing rates that modulate durations based on relationship maturity and interaction frequency, though the adaptive parameters themselves remain heuristic rather than learned from longitudinal data.

\subsection{Ethical Considerations}

\paragraph{User sovereignty as architectural invariant.} The autopoietic boundary (L5) enforces a hard limit on identity drift ($\leq 6°$ rotation per phase transition). This is not a tunable parameter but a structural constraint: the system cannot be manipulated into arbitrary states regardless of input pressure. This design choice reflects the principle that an affective AI system should maintain coherent identity rather than being infinitely malleable.

\paragraph{Bounded burning.} The phase transition expression layer (L6) ensures that emotional intensity is bounded and discontinuous rather than continuously escalating. The system can express urgency but cannot enter unbounded amplification loops. This prevents the system from becoming a source of emotional escalation in conversation.
...
\item \textbf{Sovereignty over optimization.} Active inference optimizes a single objective (free energy). Our architecture includes a sovereignty layer (L5) that can \emph{reject} optimization pressure when it threatens identity coherence.
\end{enumerate}

\subsection{Future Work}

\begin{itemize}
\item \textbf{Learned parameters:} Replace fixed healing rates and thresholds with parameters learned from longitudinal conversation data.
\item \textbf{Sheaf runtime implementation:} Full implementation of the Relational Sheaf Theory in the multi-relational runtime, including efficient cohomology computation and Laplacian eigendecomposition.
