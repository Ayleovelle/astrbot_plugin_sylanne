...
class AlphaKernel:
    ...
    def restore(cls, snapshot: dict[str, Any]) -> "AlphaKernel":
        kernel = cls(
            ...
            body=AlphaBodyState.from_dict(snapshot.get("body") if isinstance(snapshot.get("body"), dict) else {}),
            audit=dict(snapshot.get("audit") if isinstance(snapshot.get("audit"), dict) else {}),
            turns=max(0, int(snapshot.get("turns") or 0)),
            last_event=dict(snapshot.get("last_event") if isinstance(snapshot.get("last_event"), dict) else {}),
            previous_event=dict(snapshot.get("previous_event") if isinstance(snapshot.get("previous_event"), dict) else {}),
            relational_time=dict(snapshot.get("relational_time") if isinstance(snapshot.get("relational_time"), dict) else {}),
            last_decision=dict(snapshot.get("last_decision") if isinstance(snapshot.get("last_decision"), dict) else {}),
            last_guard=dict(snapshot.get("last_guard") if isinstance(snapshot.get("last_guard"), dict) else {}),
            personality=dict(snapshot.get("personality") if isinstance(snapshot.get("personality"), dict) else {}),
            moral_repair=dict(snapshot.get("moral_repair") if isinstance(snapshot.get("moral_repair"), dict) else {}),
            fallibility=dict(snapshot.get("fallibility") if isinstance(snapshot.get("fallibility"), dict) else {}),
        )
    ...
    def tick(self, event: AlphaKernelEvent | dict[str, Any] | None = None, assessment: dict[str, Any] | None = None) -> dict[str, Any]:
        ...
        self.relational_time = self._relational_time_layer(current=self.last_event, previous=previous)
        self.last_decision = self._decide()
        self.last_guard = self._guard(self.last_decision)
        return {"state": self.body, "decision": self.last_decision, "guard": self.last_guard, "surface": self.surface()}
    ...
    def _workset(self, decision: dict[str, Any], guard: dict[str, Any]) -> dict[str, Any]:
        ...
        memory_matches = self.body.recall_memory(str(self.last_event.get("text") or ""), limit=3) if self.last_event.get("text") else []
        primary = "guard" if not guard["allowed"] else decision.get("reason_code", "body")
        if primary not in {"body", "memory", "guard", "assessor", "dialogue", "personality", "attention"}:
            primary = "body"
        ...
        return build_fragment_workset(
            ...
            dialogue={"flags": list(self.last_event.get("flags", [])), "confidence": self.last_event.get("confidence", 0.0)},
            personality={"plasticity": vector_summary["plasticity"], "drift_policy": "slow"},
            body=vector_summary,
            assessor={"lane": "local", "suggestion": decision["action"], "confidence": decision["confidence"]},
            guard=guard,
            attention={"primary": primary, "weights": {"body": vector_summary["need"], "guard": guard["risk_score"], "memory": min(1.0, len(memory_matches) / 3.0)}, "pressure": flood["pressure"], "flood_policy": flood["policy"], "interests": flood["interests"]},
        )
    ...
    def _evolve_alpha_layers(self, event: AlphaKernelEvent) -> None:
        ...
        self.moral_repair = {
            ...
            "constraints": ["brief_repair_only", "no_guilt_loop", "current_user_text_priority"],
        }
        ...
        self.fallibility = {
            ...
            "claim_caution": round(max(0.0, min(1.0, (1.0 - event.confidence) + fallibility_events * 0.12)), 6),
            "constraints": ["admit_uncertainty", "correct_once", "no_performative_self_blame"],
        }
    ...
    def _affect_dynamics(self) -> dict[str, Any]:
        body = self.body.to_dict()
        repair_drive = max(body["needs"]["need_repair"], body["wound"]["repair"], body["wound"]["open"] * 0.5)
        ...
        return {
            ...
            "body_coupling": {
                ...
                "quiet_drive": round(max(body["needs"]["need_quiet"], body["immunity"]["boundary_pressure"]), 6),
            },
            "constraints": ["weak_style_modulation_only", "no_medicalized_body_claims", "no_relationship_diagnosis"],
        }

    def _group_atmosphere(self) -> dict[str, Any]:
        flags = set(self.last_event.get("flags", []))
        values = self.last_event.get("values") if isinstance(self.last_event.get("values"), dict) else {}
        ...
        return {
            ...
            "constraints": ["timing_signal_only", "no_group_mind_reading", "no_speaking_for_others"],
        }
    ...
    def _proactive_source(self, decision: dict[str, Any], guard: dict[str, Any]) -> dict[str, Any]:
        ...
        drivers = {
            "body_need": round(max(self.body.needs["need_contact"], self.body.needs["need_expression"], self.body.needs["need_repair"]), 6),
        ...
        eligible = bool("proactive" in self.last_event.get("flags", []) and guard["allowed"] and self.body.immunity.interruption_budget > 0.1)
        return {
            ...
            "reason": decision.get("reason_code", "life_rhythm") if eligible else guard.get("reason", "not_requested"),
            "constraints": ["current_user_sovereignty_first", "no_private_memory_recall", "cooldown_and_budget_required"],
        }
    ...
    def _integrated_self(self, decision: dict[str, Any], guard: dict[str, Any]) -> dict[str, Any]:
        ...
        allowed_actions = list(dict.fromkeys(action for action in allowed_actions if action not in {"wait", "hold", "withdraw"}))

        blocked_actions = ["unrequested_relationship_narration", "relationship_fact_claim", "override_current_user_text"]
        ...
        return {
            ...
            "state_index": {
                ...
                "boundary_need": round(max(self.body.immunity.boundary_pressure, 1.0 - self.body.immunity.sovereignty), 6),
                "truthfulness_guard": round(max(risk_score, 1.0 - float(self.last_event.get("confidence", 0.0) or 0.0)), 6),
            ...
            "intent_plan": {"primary_goal": primary_goal, "lanes": lanes, "current_user_text_priority": True},
            "risk": {"score": round(risk_score, 6), "safety_priority": round(max(risk_score, 1.0 - self.body.immunity.sovereignty), 6)},
            "constraints": ["current_user_text_priority", "no_raw_text", "relationship_memory_is_advisory", "no_relationship_fact_without_user_confirmation"],
        }

    def _relational_time_layer(self, *, current: dict[str, Any], previous: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "time_gap": {"seconds": gap_seconds, "label": self._gap_label(gap_seconds, bool(previous))},
            "day_relation": self._day_relation(current_time, previous_time),
            "constraints": ["internal_prompt_context_only", "no_raw_text", "does_not_override_current_user_text"],
        }
    ...
    def _event_time_payload(self, event: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"local_datetime": local_datetime, "timezone": timezone, "epoch": round(epoch, 6)}
    ...
    def _vector_summary(self) -> dict[str, float]:
        ...
        return {
            "vitality": round(min(1.0, vector["pulse.rhythm"] + vector["bloodflow.circulation"] * 0.2), 6),
            "need": round(max(vector["needs.need_contact"], vector["needs.need_expression"], vector["needs.need_repair"]), 6),
            "risk": round(max(vector["immunity.boundary_pressure"], vector["mortality.exhaustion"], vector["wound.open"]), 6),
    ...
    def _decide(self) -> dict[str, Any]:
        ...
        return {"action": action, "reason": reason, "reason_code": reason_code, "confidence": round(min(1.0, 0.35 + self.body.nerve.plasticity), 6)}

    def _risk_score(self) -> float:
        vector = self.body.state_vector()
        return round(max(vector["immunity.boundary_pressure"], vector["mortality.exhaustion"], vector["wound.open"], 1.0 - self.body.immunity.sovereignty), 6)
