...
EMBODIMENT_TRAITS = (
    "expression_drive_trait", "perception_acuity", "boundary_permeability",
    "inner_order", "relational_gravity",
)

SYLANNE_TRAITS = (
    "warmth_bias", "edge", "curiosity", "patience",
    "intimacy_gravity", "sovereignty_guard",
)
...
DRIFT_SIGNALS: dict[str, list[tuple[str, float]]] = {
    ...
    "feedback_rejected": [("expression_drive_trait", -0.6), ("relational_gravity", -0.3)],
...
class DriftSignalExtractor:
    ...
    def extract(self, result: dict[str, Any]) -> dict[str, float]:
        ...
        low_surprise_count = sum(
            1 for r in self._window
            if float(r.get("surprise", 0.0)) < 0.2
        )
...
def compute_embodiment_drift(
    ...
    for signal_name, signal_value in signals.items():
        ...
        for trait_name, weight in mappings:
            ...
            raw_delta = base_rate * signal_magnitude * weight * inertia * homeostatic * asymmetric
...
def sylanne_bounds_from_embodiment(embodiment: dict[str, TraitMemory]) -> dict[str, tuple[float, float]]:
    """Compute [min, max] bounds for each Sylanne trait from Embodiment five."""
    e = embodiment.get("expression_drive_trait", TraitMemory(0.5)).value
    p = embodiment.get("perception_acuity", TraitMemory(0.5)).value
    ...
    return {
        ...
        "sovereignty_guard": (max(0.0, 0.3 + (1 - b) * 0.2), min(1.0, 0.5 + (1 - b) * 0.5)),
    }
...
__all__ = [
    "PERSONALITY_SCHEMA_VERSION", "EMBODIMENT_TRAITS", "SYLANNE_TRAITS",
    "TraitMemory", "DriftSignalExtractor", "OscillationDetector",
    "compute_embodiment_drift", "drift_sylanne_traits", "drift_personality",
    "initial_personality", "normalize_personality", "sylanne_bounds_from_embodiment",
    "DRIFT_SIGNALS", "_LEGACY_MAP", "_REVERSE_LEGACY_MAP",
]
