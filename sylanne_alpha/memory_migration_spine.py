...
这意味着：
  - 若把部署回滚到 MEM-01 之前的构建，旧格式存量数据不会被那个版本"正确处理"，
    只会被"正确忽略"（不崩溃，但也读不到）——这是历史上一直存在的行为，不是本次
    改动引入的新风险，本次改动只是把它变成了"正确处理"。
  - 但回滚到 MEM-01 之前的构建后，**只要不去触碰旧格式数据本身**（新格式 v3 KV blob
    保持 v2 同形状的顶层键，参见 memory_system.py 的 CURRENT_SCHEMA_VERSION 注释），
    旧构建仍然能通过 key-subset 嗅探把 v3 blob 当作合法的"新版 MemorySystem 格式"
    接受并正常工作——这是刻意保留的 graceful-degrade 路径。
  - 因此"回滚地板"的真正含义是：**旧版 SylanneMemoryState / .alpha.json 救援 /
    quarantine 侧车这几条 MEM-01 新增的读取路径，只有本次构建及之后的版本才会
    走通**；回滚到更早的构建不会导致数据损坏或误清空，只会导致这几条新增的
    救援能力暂时不可用（等同于回到 MEM-01 之前的已知行为，不是新的退化）。
...
ROLLBACK_FLOOR_BUILD: Final[str] = "MEM-01（金档往返基座 + 迁移主脊）"
ROLLBACK_FLOOR_NOTE: Final[str] = (
    "回滚不得早于本次 MEM-01 构建对应的这次提交。"
    "早于此点的构建，旧版 SylanneMemoryState / .alpha.json 救援 / quarantine "
    "侧车这几条读取路径不可用（等同回到 MEM-01 之前的既有行为，不是新退化）；"
    "v3 KV blob 因保持 v2 同形状顶层键，仍可被更旧构建的 key-subset 嗅探接受。"
)
