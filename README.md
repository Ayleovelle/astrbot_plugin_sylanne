...
| 用户控制 | 暂停、重置、退出及敏感能力授权均受硬门控。 | 用户随时可以收回授权。 |
...
2. 开启长期记忆或 Embedding 辅助召回，检查重启后的恢复结果。
...
主回复模型可在输出中生成带本轮 nonce 的隐藏边界。存在合法边界时按边界发送；没有 marker 时以正文显式换行回退。代码块、表格和列表保持完整，孤立标点并入相邻文本；marker 异常、越界或位于保护区时，移除 marker 后整条发送。超长内容仍按平台限制安全切分。
