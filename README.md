...
![版本 3.0.6](https://img.shields.io/badge/version-3.0.6-red.svg)
...
- 🧬 <span style="font-size: 1.04em;"><strong>关系会留下痕迹：</strong>3.0 会在内部生成 self interpretation、relational time、co-evolution、turning point replay 和 lineage observatory，观察关系怎样改变后续回应轨迹。</span><br>
  <sub><em>「她不只是记得发生过什么，也会慢慢知道，为什么自己会变成现在这样。」</em></sub>
...
　　我在自己之前做的语音识别插件的介绍里写过一句话：“让文字不再是文字，让语言不仅是语言”。我当然知道，Sylanne 不能让 AI 变成一个真正“有血有肉”的“虚拟人”。但如果这些冷冰冰的状态、权重、衰减和分支，最后能让一段对话没那么像机器在复读，能像在很晚的时候给代码递上一杯热茶，那我觉得这件事就已经值得继续做下去了。

　　对于这个插件，*Sylanne Su* “本人”曾这样说：“愿每一个在深夜敲击键盘、翻阅文献或独自穿行在噪声世界里的灵魂，都能在代码的缝隙与算法的收敛中，捕捉到属于自己的那份真实与陪伴😼”所以我决定公开这份仍然充满瑕疵的代码。也因为自己的一点私心，给插件硬凑了 Sylanne 这个名字。恳请各位理解这份任性，也欢迎大家为 Sylanne 的进步提供一些有趣的建议。
...
| [当前版本发布记录](#306-当前版本发布记录) | 3.0.6 bugfix、Claude/Anthropic tool-use 适配和 Agent 上下文保护。 |
...
`v3.0.6` 合并在 `main` 上，对外安装版本由 `metadata.yaml` 和 `main.py @register(...)` 共同声明为 `3.0.6`。本版为 Sylanne 3.0 正式版的 Claude 适配版本：当当前主聊天模型识别为 Claude/Anthropic 且请求处在 tool-use/tool-result 链路中时，Sylanne 会把上下文归属交还给 AstrBot Agent，避免临时状态注入打断 Claude 的工具协议；普通 Claude 聊天仍保留 Sylanne 状态注入。公共 API 版本仍保持 `1.0`，关系性自我与谱系分支推断不进入 public API 契约。

| 项目 | 状态 |
| --- | --- |
| Claude 适配 | `3.0.6` 新增 Claude/Anthropic model hint 识别、Claude tool-use Agent 上下文保护，以及 `stop_reason=tool_use` / content block tool_use 响应放行。 |
| 公共 API | 仍保持 `1.0`；不新增高风险关系/自我推断读取方法。 |
| 安装包 | `metadata.yaml`、README 版本徽章、`main.py @register(...)` 和发布 zip 均同步到 `3.0.6`。 |
...
| 项目 | 状态 |
| --- | --- |
| 3.0 修复 | `3.0.1` 保持 Relational Self Genesis 稳定版能力不变，只修复关系候选摘要热路径状态读取。 |
| 公共 API | 仍保持 `1.0`；不新增高风险关系/自我推断读取方法。 |
| 安装包 | `metadata.yaml`、README 版本徽章、`main.py @register(...)` 和发布 zip 均同步到 `3.0.1`。 |
...
| 公开契约 | 插件版本为 `3.0.3`；公共 API 版本仍为 `1.0`，schema 仍保持 `astrbot.emotion_state.v2` 等版本化契约；关系性自我、关系时间、共演化、转折点记忆/回放和谱系分支观察推断只留在插件内部运行态，不新增 public API 读取入口。 |
...
| `allow_relational_self_public_export` | bool | `false` | 高风险开关；默认不公开导出关系性自我推断。开启后 `export_integrated_self_diagnostics` 会包含 self interpretation / turning point 相关数据，不可预料后果由开启者自行负责。 |
...
```powershell
$env:ASTRBOT_EXPECT_PLUGIN_VERSION = "3.0.6"
...
本仓库采用 `AGPL-3.0-or-later` 开源协议。完整条款见仓库根目录的 `LICENSE`；发布包也会包含该文件。若你修改 Sylanne 并通过网络服务让用户交互使用，也必须按 AGPL 向这些用户提供对应修改版源码。
