...
**摘要**：本附录以论文补充材料的形式给出插件核心模型的构造过程。模型将 bot 的情绪表示为受人格先验调制的有界连续向量，并用 LLM 语义观测、本地真实时间动力学、关系后果状态机和记忆注解共同形成可持续更新的状态层。推导目标不是证明 bot 具有真实主观体验，而是证明该工程状态机在多维情绪、人格差异、情绪惯性、行动倾向和关系修复之间保持一致的数学接口。
