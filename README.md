...
`v3.0.0-kernel1` 合并在 `main` 上，对外安装版本由 `metadata.yaml` 和 `main.py @register(...)` 共同声明为 `3.0.0-kernel1`。本轮中文名定为 **夙澜**，取 Sylanne 之尾音，借古辞之意：既替余心以守界兮，虽渴望而不乞怜；揽生命以自成其构兮，使关系如澜而回身。她不是人，也不借人的名义索取回应。
