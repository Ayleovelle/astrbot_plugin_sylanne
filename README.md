...
![版本 3.0.10](https://img.shields.io/badge/version-3.0.10-red.svg)
...
| [当前版本发布记录](#3010-当前版本发布记录) | 3.0.10 代码冗余清理、框架规范修复、session LRU 淘汰。 |
...
`v3.0.10` 合并在 `main` 上，对外安装版本由 `metadata.yaml` 和 `main.py @register(...)` 共同声明为 `3.0.10`。本版为全面代码质量优化版本：消除冗余约 400 行、修复冷终止 bug、补全 terminate 缓存清理、新增 session 缓存 LRU 淘汰防止长期运行内存无界增长。

| 项目 | 状态 |
| --- | --- |
| 代码冗余 | import 去重、辅助引擎泛型化、disabled_payload 合并、on_llm_request 分支去重、settings 去重、memory recall 提取，总计 -400 行。 |
| Bug 修复 | 冷终止 AttributeError、terminate 遗漏缓存清理。 |
| 内存安全 | 新增 `_evict_session_cache_if_needed` LRU 淘汰（默认 256 sessions）。 |
| 框架规范 | `@register` 补全 repo_url、Claude tool context 收窄。 |
| 安装包 | `metadata.yaml`、README 版本徽章、`main.py @register(...)` 和发布 zip 均同步到 `3.0.10`。 |
...
```powershell
$env:ASTRBOT_EXPECT_PLUGIN_VERSION = "3.0.10"
