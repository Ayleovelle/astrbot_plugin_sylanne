...
| **L5** | MoE-HGT 决策融合 | MoE 多专家混合 + 异构图 Transformer，scar token 对数压缩，负载均衡，人格派生全参数 | 0.4ms |
...
> 三种条件下人格向量的漂移轨迹。反复受伤：感知锐度↑ 边界通透↓（变敏感变封闭）。持续接纳：表达驱力↑ 关系引力↑（变外向变合作）。跨关系矛盾：内在秩序↓（维持不了一致性时条理崩塌）。漂移是缓慢的、有阻尼的（Dual-EMA），单次事件改变不了你是谁。
