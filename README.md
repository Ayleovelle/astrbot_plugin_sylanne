...
<p align="center">
  <a href="https://github.com/Ayleovelle/astrbot_plugin_sylanne/releases"><img src="https://img.shields.io/badge/version-2.3.0-red.svg" alt="version 2.3.0"></a>
...
> **Sylanne-Embodiment：不可逆的关系计算引擎 + 自我进化认知体 + 关系性自证心智。** 不模拟情绪标签——对话在躯体上留疤，沉默中积压，关系里长出不可撤销的形状。认知内核编排出一颗会自我进化的心智：白天反应式微调，睡眠期反思沉淀，跨重启累积学习。2.3.0 根治了主动消息的三链重复、堵住了内容片段泄露通道，底层引擎升级至 SylannEngine 2.3.2。
...
**3. 引擎升级 SylannEngine 2.3.2**

vendored sylanne_core 2.0.0 → 2.3.2：2.3.0 新增 deterministic_fusion / pel_core / telemetry、移除 resonance-field 死栈；2.3.1 做畸形 LLM 输入硬化（非 dict JSON / null 字段 / 跨档位快照维度对齐 / NaN·溢出守卫，新增 `_numeric`）；2.3.2 专项加固进程级引擎共享机制（completeness 审计补 2 blocker+4 major、并发与鲁棒性），新增 `_config_store` / `_identity` / `_rendezvous` / `_assessor_llm` 内部模块。公共导出面（43 符号）逐字不变，对插件零接口变更。
