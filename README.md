...
| `sylanne_memory_record_embedding_min_interval_seconds` | float | `300.0` | 控制写入侧为新记忆生成 semantic embedding 的最低频率；默认同一会话至少间隔 5 分钟，避免密集聊天时持续调用 Embedding Provider。 |
| `sylanne_memory_record_embedding_max_per_flush` | int | `1` | 控制一次 idle flush 最多为多少条新记忆生成向量；设为 0 可保留向量召回但停止写入侧新建向量。 |
