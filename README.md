...
![版本 3.0.5](https://img.shields.io/badge/version-3.0.5-red.svg)
...
| [当前版本发布记录](#305-当前版本发布记录) | 3.0.5 optimization、cgroup memory working set 口径和 proactive dispatch 预算诊断。 |
...
`v3.0.5` 合并在 `main` 上，对外安装版本由 `metadata.yaml` 和 `main.py @register(...)` 共同声明为 `3.0.5`。本版为 Sylanne 3.0 正式版的优化版本：在 3.0.4 低端容器保护之上，把 cgroup 内存压力改为 working set 口径，扣除可回收 page cache 后再决定降载，并把 proactive dispatch 纳入分功能低配预算诊断。公共 API 版本仍保持 `1.0`，关系性自我与谱系分支推断不进入 public API 契约。

| 项目 | 状态 |
| --- | --- |
| 3.0 优化 | `3.0.5` 保持 Relational Self Genesis 稳定版能力不变，优化 cgroup memory working set 压力采样，减少 page cache 导致的低端容器误降载，并补齐 proactive dispatch 预算可观测性。 |
| 公共 API | 仍保持 `1.0`；不新增高风险关系/自我推断读取方法。 |
| 安装包 | `metadata.yaml`、README 版本徽章、`main.py @register(...)` 和发布 zip 均同步到 `3.0.5`。 |
...
```powershell
$env:ASTRBOT_EXPECT_PLUGIN_VERSION = "3.0.5"
