...
本版本将 2.5.0 grey 系列验证的改动收口到 stable 通道。Embodiment 命名版本与后文旧数字版本属于不同谱系。

### Added

- 按身份跨群、跨私聊延续的记忆、关系状态与人格快照；默认关闭，并受身份、范围和可见性门控。
- 由生活模拟驱动的 QQ 空间说说；默认关闭、默认逐条由主人确认，并保留隐私净化及日/周频率限制。
- 主回复模型在同一次生成中标注语义节拍，无需额外分段 LLM。
- WebUI 提供会话、认知、路由、边界、表达、反馈等运行观测入口。

### Changed

- 无合法 marker 时按显式换行分段；代码块、表格和列表保持完整，孤立标点并入相邻内容。
...
- stable 包版本身份为 `2.5.0`；v3 影子路径在该包中关闭，不接管请求、回复或状态写入。

### Fixed

- TTS、媒体或其他装饰完成后才决定发送所有权，组件化结果不会提前重复发送文字。
- 即时分段无法安全启动时，会在正文发送前回退至 AstrBot，避免丢失、重复或残留失效发送。
