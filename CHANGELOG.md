...
## [Embodiment-2.5.0] - 2026-07-26

本版本将 2.5.0 grey 系列完成验证的改动收口到 stable 通道。

### Added

- 提供跨群记忆、QQ 空间说说和即时聊天接管等可选能力，默认关闭。
- WebUI 提供配置、会话切换、认知状态、路由、边界、表达和反馈等实时观测数据。
- 回复模型可在同一次生成中标注语义节拍，不增加第二次 LLM 请求。

### Changed

- 无合法 marker 时按模型正文的显式换行分段；代码块、表格和列表保持完整，纯标点片段并入相邻内容。
- 配置页以聊天模型、共享辅助文本模型和 Embedding 模型为主；功能专用 Provider 收入高级覆盖区。
- 前台即时判断改为本地 v2core 计算，不再同步调用独立评估 LLM。
- v2core 成为唯一逐轮认知内核，ResonanceSpine 成为唯一身体计算后端。
- Agent 基础设施收口为 `AUTONOMOUS` 生命周期，只保留生活模拟、反射学习、反思和巩固。

### Fixed

- 统一框架保存和直发工具路径的历史最终化，第三方 Agent、TTS 与工具接管后的真实 user/assistant 轮次都按最终投递结果保存。
- 历史过滤使用 Provider 调用期间的临时视图，保存前恢复原始 `request.contexts`，并在视图变化后重新计算 token。
- 修复分段取消、首段或后续发送失败时未发送正文丢失，以及 TTS/媒体接管后文字重复发送。
- 修复 WebUI 会话、路由、时序、Resonance/SKIP 统计和运行态字段未连接到后端的问题。
- 纯文本轮次不再开放“自行发送且无返回值”的搜索工具，避免工具结束后没有可交付回复。
- 保留中文记忆整理、AstrBot 4.26 钩子、覆盖安装热重载和上下文隔离等 grey 系列修复。

### Removed

- 删除未接入生产路径的 V1 迁移存储、旧核心快照导入器、冻结计算后端和响应式 Agent 总线。
- 删除同步评估 Provider 配置、旧 Provider 专用请求改写和失效的回滚语义。

### Release boundary

- 安装包使用 `stable` 构建通道，版本身份统一为 `2.5.0`，文件名不带 grey 后缀。
- v3 影子路径在 stable 包中关闭，不参与用户请求、回复或状态写入。
