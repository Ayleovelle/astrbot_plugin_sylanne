...
- 开源协议从 `GPL-3.0-or-later` 调整为 `AGPL-3.0-or-later`，要求网络服务中的修改版也向交互用户提供对应源码，避免 Sylanne 被闭源化托管。

### 修复

- 修复实时聊天接管生图结果时，媒体发送完成后同步等待 delivery context KV 持久化可能放大磁盘 I/O 卡顿的问题。
- 已送达实时回复改为先在内存中释放临时上下文，再把 delivery context 保存调度到后台任务，避免图片发送热路径被磁盘写阻塞。

### 验证

- 新增实时媒体发送后延后 KV 写入的回归测试，锁定图片和文本先完成发送，再执行 delivery context 持久化。
- 重新运行实时聊天生命周期测试与 `main.py` 编译检查。
