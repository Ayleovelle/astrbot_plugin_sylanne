...
本文件用于 AstrBot 插件市场和管理页展示更新内容。功能、配置和部署说明见 `README.md`。
...
- 历史清理改为 provider 调用期间的临时视图，不再改写 `request.contexts`；视图变化时重新估算 token，并在 AstrBot 持久化前恢复原始历史，避免错误压缩造成上下文永久丢失。
- 删除 Hajide 专用配置和请求改写分支；所有 Provider 使用同一条 `system_prompt` 状态注入路径。
- 修复 TTS 或发言工具已接管回复时仍发送原始文字的问题，并在最终发送前清理内部控制标记。
...
> 灰测版。跨群记忆 / QQ空间说说 / 即时聊天接管三大功能仍默认全关；本版重点验证模型原生智能分段、LLM 配置收口与 **v3 零-LLM 影子认知**。请勿将此包当作 stable 版使用。

### 💬 模型原生智能分段：按语义节拍发，不再靠本地猜句子

- 分段边界由同一次主回复模型直接标注，不新增第二次 LLM 请求，也不再用标点长度状态机决定聊天气泡。
- 最多插入 5 个语义边界（形成不超过 6 个自然节拍）；代码、URL、表格等结构化内容禁止切分。错误 nonce、畸形或未闭合标记全部清洗并安全退回整段文本，不会把控制标记写进消息或历史。
- 首段等待期间取消、首段发送失败或后续发送失败时，未发送正文都会精确保留，避免闪退或网络异常后整句消失。

### 🧠 LLM 配置收口：常用只留一处，特殊需求再高级覆盖

- 配置页以“当前聊天模型 + 共享辅助文本模型 + Embedding 模型”为主；即时评估、生活模拟、关系分类、说说草稿等独立 Provider 收进高级覆盖。
- Embedding 只从专用注册表选择：只有一个时自动使用，多个时明确要求选择；独立 WebUI 与内嵌 WebUI 使用同一份路由信息。
- 后台评估默认不会悄悄继承聊天模型产生额外付费请求；只有显式选择功能模型、共享辅助模型或打开真实评估开关才会启用。Provider 内部抛出 `TypeError` 时也不会重放已可能计费的请求。

### 🌗 新增：v3 影子认知层（默认关；灰测渠道后台静默运行）

一层独立的、纯确定性（不调用任何 LLM）的旁挂认知体，与现有 v2 并行、只观察不接管：

- **对现网零影响、隔离证过**：v2 的回复 / prompt / 历史 / 记忆 / body 快照 / 工具与 LLM 调用次数逐字节不变；影子的计算跑在独立单线程执行器上（绝不占用事件循环），七个隔离计数器在正常/队满/超时/仓库失败/畸形输入/重载/关闭下恒为零；任何异常都封在影子内、绝不外泄进 v2。
- **在使用中静默学习**：用你下一轮的自然反应（回得多快、多长、语气冷暖）当偏好信号，后台微调它自己的世界模型与偏好——不依赖"这条回复好不好"的人工标注。这条学习通道对回复没有任何作用，纯粹是在为将来"越用越懂你"积累可验证的证据。
- **崩溃安全 + 严格边界**：独立命名空间与文件、CAS 事务持久化、跨重启不丢不分叉；绝不写入 v2 的任何状态。
- **配置面零改动**：不新增任何用户可选项；开/关由构建渠道决定（灰测渠道后台激活、正式渠道关闭）。稳定版发布不含激活的影子。

### 说明

三大高风险功能（跨群记忆 / QQ空间说说 / 即时聊天接管）仍默认全关、逐项开启才生效。v3 仍是灰测影子通道；G3/G4 必须依赖真实灰测数据，当前包不据此宣称 stable 就绪。
